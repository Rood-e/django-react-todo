        ...
        <div className='relative min-h-screen'>
            ...
            {showDeleteModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
                    <div className="bg-white dark:bg-slate-900 w-full max-w-md p-8 rounded-[2.5rem] shadow-2xl border border-slate-100 dark:border-slate-800 animate-in zoom-in-95 duration-300">
                        ...
                        <h2 className="text-2xl font-black text-center text-slate-900 dark:text-white mb-2">Sei sicuro?</h2>
                        <p className="text-slate-500 dark:text-slate-400 text-center mb-8">Questa azione è irreversibile. Tutti i tuoi dati verranno persi.</p>
                        <div className="flex flex-col gap-3">
                            <button onClick={deleteUser} className="w-full py-4 bg-red-600 hover:bg-red-700 text-white font-black uppercase tracking-widest rounded-2xl transition-all">Elimina ora</button>
                            <button onClick={() => setShowDeleteModal(false)} className="w-full py-4 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 font-bold rounded-2xl hover:bg-slate-200 dark:hover:bg-slate-700 transition-all">Annulla</button>
                        </div>
                    </div>
                </div>
            )}
